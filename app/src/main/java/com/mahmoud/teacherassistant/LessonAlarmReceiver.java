...
public class LessonAlarmReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        String lessonId = intent.getStringExtra(EXTRA_LESSON_ID);

        String title = "⏰ تذكير بالحصة";
        StringBuilder body = new StringBuilder("الحصة تبدأ قريبًا");
        if (group != null && !group.trim().isEmpty()) body.append(" • مجموعة ").append(group);
        if (time != null && !time.trim().isEmpty()) body.append(" • الساعة ").append(time);
        ...
        if (nm != null) {
            int id = NotificationScheduler.stableId(lessonId == null ? "lesson" : lessonId);
            nm.notify(id, notification);
        }

        // Schedule the next weekly occurrence. Never let a scheduling error
        // prevent the current notification from being delivered.
        try {
            NotificationScheduler.scheduleAll(context);
